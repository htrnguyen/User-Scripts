...
// @namespace    https://github.com/htrnguyen/User-Scripts/Coursera-Auto-Subtitle
// @version      1.0
...
// @license      MIT
// @icon         https://github.com/htrnguyen/User-Scripts/raw/main/Coursera-Auto-Subtitle/coursera-auto-subtitle-logo.png
// ==/UserScript==
...
;(function () {
    ...
    function createDraggableIcon() {
        ...
        icon.src =
            'https://github.com/htrnguyen/User-Scripts/raw/main/Coursera-Auto-Subtitle/coursera-auto-subtitle-logo.png'
        ...
        document.addEventListener('mousemove', (event) => {
            if (isDragging) {
                ...
                icon.style.left = `${Math.max(
                    0,
                    Math.min(window.innerWidth - icon.offsetWidth, newLeft)
                )}px`
                icon.style.top = `${Math.max(
                    0,
                    Math.min(window.innerHeight - icon.offsetHeight, newTop)
                )}px`
    ...
    function updateMenuPosition() {
        ...
        if (iconRect.left + icon.offsetWidth + menuWidth > window.innerWidth) {
            ...
            iconRect.top + icon.offsetHeight + menuHeight >
            window.innerHeight
        ) {
    ...
    function showMenu() {
        ...
        toggleButton.textContent = isSubtitlesEnabled
            ? 'Disable Subtitles'
            : 'Enable Subtitles'
            ...
            if (activeCue && isSubtitlesEnabled) {
                ...
                if (CONFIG.translateSubtitles && originalText) {
                    translateSubtitles(originalText, targetLanguage)
                        .then((translatedText) => {
                            if (subtitleDisplayElement) {
                                subtitleDisplayElement.textContent =
                                    translatedText
                            }
                        })
                        .catch(() => {})
